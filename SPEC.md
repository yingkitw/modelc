...
- **`POST /v1/embeddings`** — OpenAI-compatible embeddings endpoint. Request JSON: `{ "input": "..." }` or `{ "input": ["...", "..."] }`, optional `encoding_format: "float"` (default) or `"base64"`, optional `model`. Response: `{ "object": "list", "data": [{ "object": "embedding", "embedding": [f32, ...] | "base64...", "index": 0, "dimensions": N }, ...], "model": "...", "usage": { "prompt_tokens": N, "total_tokens": N } }`. Uses the same embedding logic as `/embeddings`.
...
- **`GET /api/version`** — exposes CLI version and git SHA for orchestration/health checks. JSON: `{ "version": "0.1.5", "git_sha": "abc123" }`. `git_sha` is captured at compile time by `build.rs` via `git rev-parse --short HEAD`.
...
- **Authentication, rate limiting, and backpressure** — `modelc run` accepts `--api-key <key>`, `--rate-limit <N>` (requests per minute per client IP), and `--max-concurrent <N>` (in-flight inference limit). When `--api-key` is set, every request except `/health` and `/info` must include an `Authorization: Bearer <key>` header; missing or incorrect tokens return `401 Unauthorized`. When `--rate-limit` is set, a per-IP token bucket (`src/serve/auth.rs`) enforces the cap; exceeded requests return `429 Too Many Requests`. When `--max-concurrent` is set, a `tokio::sync::Semaphore` limits concurrent inference requests; saturated requests receive `503 Service Unavailable`. `/health`, `/info`, and `/metrics` are always exempt so load-balancer liveness probes and observability continue to work.
- **Graceful shutdown** — `modelc run` (and compiled binaries generated by `modelc compile`) catch `SIGINT`/`SIGTERM` via `tokio::signal`. `axum::serve` uses `with_graceful_shutdown` to stop accepting new connections and drain in-flight requests before exiting. SSE streams cooperate via `CancelOnDrop`, whose `Drop` trips a cancellation flag when the client disconnects.
