...
fn build_gpt2_forward(_model: &Model) -> String {
    // Simplified GPT2 forward: embedding lookup + stacked transformer layers + output projection.
    ...
    for layer in 0..n_layers {
        ...
        // Simplified: apply layer norm + attention + MLP (weights decoded inline).
    ...
    // Final output projection via wte (shared with embedding).
...
fn build_llama_forward(_model: &Model) -> String {
    // Simplified LLaMA forward: RMS norm + RoPE attention + MLP layers.
    r#"fn forward(state: &AppState, input: &[f32]) -> Vec<f32> {
    let mut hidden = input.to_vec();
    ...
    for layer in 0..n_layers {
        ...
        // Simplified attention via q_proj + k_proj + v_proj.
        if let Some(_) = state.tensors.get(&format!("{}.self_attn.q_proj.weight", prefix)) {
            ...
            // Simplified: hidden = q + v (no RoPE, no softmax attention for codegen stub).
            hidden = q.iter().zip(v.iter()).map(|(a, b)| a + b).collect();
        }
        // MLP: gate_proj -> SiLU + up_proj.
